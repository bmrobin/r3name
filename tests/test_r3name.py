...
class TestSub:
    def test_basic_replacement(self, mkfiles, cli, lsnames, tmp_path):
        mkfiles("old_alpha.txt", "old_beta.txt")
        result = cli("--sub", "old", "new", "-y")
        assert result.returncode == 0
        assert lsnames(tmp_path) == {"new_alpha.txt", "new_beta.txt"}

    def test_no_match_is_noop(self, mkfiles, cli, lsnames, tmp_path):
        mkfiles("alpha.txt")
    ...
    def test_dry_run_does_not_rename(self, mkfiles, cli, lsnames, tmp_path):
...
class TestStrip:
    def test_strip_leading_trailing_dashes(self, mkfiles, cli, lsnames, tmp_path):
        mkfiles("--hello--.txt", "--world--.txt")
        result = cli("--strip", "-", "-y")
        assert result.returncode == 0
        assert lsnames(tmp_path) == {"hello--.txt", "world--.txt"}

    def test_strip_spaces(self, cli, lsnames, tmp_path):
    ...
    def test_strip_multiple_chars(self, mkfiles, cli, lsnames, tmp_path):
...
class TestCase:
    def test_lower(self, mkfiles, cli):
    ...
    def test_upper(self, mkfiles, cli):
        mkfiles("hello_world.txt")
    ...
    def test_title(self, mkfiles, cli):
        mkfiles("hello world.txt")
    ...
    def test_extension_preserved(self, mkfiles, cli, lsnames, tmp_path):
        mkfiles("report.TXT")
        result = cli("--case", "lower", "-y")
        assert result.returncode == 0
...
class TestNumber:
    def test_default_numbering(self, mkfiles, cli, lsnames, tmp_path):
    ...
    def test_custom_start_and_pad(self, mkfiles, cli, lsnames, tmp_path):
        mkfiles("alpha.txt")
        result = cli("--number", "--num-start", "5", "--num-pad", "3", "-y")
        assert result.returncode == 0
        assert any(n.startswith("005") for n in lsnames(tmp_path))

    def test_custom_prefix_and_sep(self, mkfiles, cli, lsnames, tmp_path):
        mkfiles("track.txt")
...
class TestScopeFilters:
    def test_files_only_skips_directories(self, mkfiles, cli, lsnames, tmp_path):
        mkfiles("old_file.txt")
    ...
    def test_dirs_only_skips_files(self, mkfiles, cli, lsnames, tmp_path):
        mkfiles("old_file.txt")
        (tmp_path / "old_dir").mkdir()
    ...
    def test_files_and_dirs_only_are_mutually_exclusive(self, cli):
...
class TestExtFilter:
    def test_only_matching_extension_renamed(self, mkfiles, cli, lsnames, tmp_path):
    ...
    def test_ext_with_leading_dot(self, mkfiles, cli, lsnames, tmp_path):
        mkfiles("old_doc.txt", "old_img.png")
        result = cli("--sub", "old", "new", "--ext", ".txt", "-y")
        assert result.returncode == 0
        assert "new_doc.txt" in lsnames(tmp_path)
...
class TestHidden:
    def test_hidden_files_excluded_by_default(self, mkfiles, cli, lsnames, tmp_path):
    ...
    def test_hidden_flag_includes_hidden(self, mkfiles, cli, lsnames, tmp_path):
...
class TestRecursive:
    def test_recursive_renames_in_subdirectories(self, mkfiles, cli, lsnames, tmp_path):
    ...
    def test_non_recursive_leaves_subdirectory_files_untouched(
        self, mkfiles, cli, lsnames, tmp_path
    ):
        subdir = tmp_path / "subdir"
        subdir.mkdir()
        mkfiles("old_nested.txt", directory=subdir)
...
class TestConflicts:
    def test_existing_target_is_skipped(self, mkfiles, cli, lsnames, tmp_path):
...
class TestSafety:
    def test_no_transform_exits_with_error(self, mkfiles, cli):
        mkfiles("file.txt")
        result = cli()
        assert result.returncode != 0

    def test_transform_producing_path_separator_exits(self, mkfiles, cli):
        mkfiles("file.txt")
        result = cli("--sub", "file", "dir/file", "-y")
        assert result.returncode != 0

    def test_transform_producing_empty_name_exits(self, mkfiles, cli):
        mkfiles("abc.txt")
...
class TestUndo:
    def test_undo_reverses_rename(self, mkfiles, cli, lsnames, tmp_path):
        mkfiles("old.txt")
        cli("--sub", "old", "new", "-y")
        assert "new.txt" in lsnames(tmp_path)
    ...
    def test_undo_manifest_written_after_rename(self, mkfiles, cli, manifest_path):
        mkfiles("old.txt")
        cli("--sub", "old", "new", "-y")
    ...
    def test_undo_manifest_deleted_after_full_undo(self, mkfiles, cli, manifest_path):
        mkfiles("old.txt")
        cli("--sub", "old", "new", "-y")
        cli("--undo", "-y")
        assert not manifest_path.exists()

    def test_undo_dry_run_does_not_rename(self, mkfiles, cli, lsnames, tmp_path):
        mkfiles("old.txt")
        cli("--sub", "old", "new", "-y")
        result = cli("--undo", "-n")
        assert result.returncode == 0
    ...
    def test_undo_no_manifest_reports_gracefully(self, cli):
        result = cli("--undo", "-y")
        assert result.returncode == 0
        assert "No undo manifest" in result.stdout

    def test_undo_skips_missing_source(self, mkfiles, cli, tmp_path):
        mkfiles("old.txt")
        cli("--sub", "old", "new", "-y")
    ...
    def test_undo_skips_conflicting_target(self, mkfiles, cli, tmp_path):
        mkfiles("old.txt")
        cli("--sub", "old", "new", "-y")
    ...
    def test_undo_cannot_be_combined_with_transforms(self, cli):
        result = cli("--undo", "--sub", "a", "b")
        assert result.returncode != 0

    def test_undo_reverses_multiple_files(self, mkfiles, cli, lsnames, tmp_path):
